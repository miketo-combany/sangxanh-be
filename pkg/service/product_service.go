...
func (s *productService) GetProductById(
	...
	if err := s.db.DB.
		From("products").
		Select("id,name,price,content,image_detail,description,product_code,questions,category_id,thumbnail,discount,discount_type,metadata,categories!inner(id,name),created_at,updated_at").
		Eq("id", id).
		IsNull("deleted_at").
